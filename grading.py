...
class User(object):
	...
	def adjust(self):
		if self.name == "Max Marks":
			self.adjusted = ["Converted to 100" for i in range(len(self.max_marks))]
			self.total = "total"
		...
		for i in range(len(self.max_marks)):
			tmp = float(self.marks[i])*float(User.weightage[i])/float(User.max_marks[i])
			tmp = round(tmp, 2)
...
class inputObj(object):
	...
	def make_temp(self, filename = "TEMP.csv"):
		self.temp = csvhandler(filename, headers = self.raw_data.headers + self.raw_data.headers[3:])
	...
	def grade_sort_total(self, filename="grade_sort_total.csv"):
		self.grade_sort_total = csvhandler(filename, headers = self.raw_data.headers + self.raw_data.headers[3:])
		self.grade_sort_total.output_data = dcopy(self.temp.output_data[2:])
		self.grade_sort_total.output_data.sort(key = lambda x: x[19], reverse = True)
		c = 1
		for i in self.grade_sort_total.output_data:
			if c == 1:
				i.append(0)
			else:
				i.append(round(self.grade_sort_total.output_data[c-2][-2] - i[-1], 2))
			i[0] = c
			c+=1

		self.grade_sort_total.output_data = self.temp.output_data[:2] + self.grade_sort_total.output_data
	...
	def grade_sort_roll(self, filename = "grade_sort_roll.csv"):
		self.grade_sort_roll = csvhandler(filename, headers = self.raw_data.headers + self.raw_data.headers[3:])
		self.grade_sort_roll.output_data = dcopy(self.temp.output_data[2:])
		self.grade_sort_roll.output_data.sort(key = lambda x: x[1], reverse = False)
		c = 1
		for i in self.grade_sort_roll.output_data:
			if c == 1:
				i.append(0)
			else:
				i.append(round(self.grade_sort_roll.output_data[c-2][-2] - i[-1], 2))
			i[0] = c
			c+=1

		self.grade_sort_roll.output_data = self.temp.output_data[:2] + self.grade_sort_roll.output_data
	...
	def make_grades(self):
		...
		for i in range(2, size):
			...
			self.grades[self.grade_sort_total.output_data[i][1]] = self.iapc[d][0]
			c+=1
			self.count[d]+= 1
